classify <- function (data, method = c("svm", "bagsvm", "randomforest", "cart"), 
                      normalize = c("deseq", "none", "tmm"), deseqTransform = c("vst", "voomCPM"), 
  ...
  result = new("MLSeq", confusionMat = confM, trained = train, 
               method = method, normalization = normalize, deseqTransform = deseqTransform, 
               ref = ref)
  result
}
