...
cart <- classify(data = data.trainS4, method = "rpart", normalize = "deseq",
	transformation = "vst", ref = "T", tuneLength = 10,
	control = trainControl(method = "repeatedcv", number = 5, repeats = 3))
cart
@

After running the code given above, we obtain the results in MLSeq class. CART successfully fits a model with \Sexpr{round(cart@confusionMat$overall[1]*100, 2)}\% true classification accuracy.
...
rf <- classify(data = data.trainS4, method = "rf", normalize = "deseq",
...
cart.tbl <- table(pred.cart$Predictions, relevel(data.testS4$condition, 2))
rf.tbl <- table(pred.rf$Predictions, relevel(data.testS4$condition, 2))
@

<<chunk19_2>>=
table(pred.cart$Predictions, relevel(data.testS4$condition, 2))
table(pred.rf$Predictions, relevel(data.testS4$condition, 2))
@
